 ...
 * @param {Object} storyEntry - The story entry from JSON
 * @param {string} exportedField - The field name to check for export status
 * @returns {boolean}
 */
function shouldHaveSceneImage(storyEntry, exportedField) {
    const codeId = storyEntry['Code ID']
    const exportedValue = storyEntry[exportedField]
 ...
 * @param {string} sceneFolder - Name of the scene folder
 * @returns {Set<string>} Set of available image filenames (without extension)
 */
function getAvailableSceneImages(sceneFolder) {
    ...
    try {
        ...
        console.error(
            chalk.red(
                `❌ Error reading scene folder ${sceneFolder}: ${error.message}`
            )
        )
    }
 ...
 * @param {string} jsonFile - Name of the JSON file
 * @returns {Array} Parsed story data
 */
function loadStoryData(jsonFile) {
    ...
    try {
        ...
        console.error(
            chalk.red(`❌ Error reading ${jsonFile}: ${error.message}`)
        )
        return []
    }
}
 ...
 * @param {Object} config - Dinosaur configuration
 * @returns {Object} Results object with missing images and statistics
 */
function checkDinosaurScenes(config) {
 ...
 * @param {Array} results - Results from all dinosaur checks
 */
